...
import org.fede.pruebas.exceptions.InteresadoNoEncontradoException;
import org.fede.pruebas.exceptions.InteresadoRestringidoException;
import org.fede.pruebas.exceptions.LicenciaVencidaException;
...
public class InteresadoService {
    ...
    public InteresadoDto updateInteresado(Integer id, InteresadoDto dto) {
        Interesado interesado = interesadoRepository.findById(id)
                .orElseThrow(() -> new InteresadoNoEncontradoException("El interesado no se encontro con ese id: " + id));
    ...
    public void validarInteresado(Integer idInteresado) {
        Interesado interesado = interesadoRepository.findById(idInteresado)
                .orElseThrow(() -> new InteresadoNoEncontradoException("Interesado no encontrado"));

        // Validar que la licencia no este vencida
        if(interesado.getFechaVenLicencia().isBefore(LocalDateTime.now())) {
            throw new LicenciaVencidaException("la licencia del interesado esta vencida");
        }

        // Validar que el interesado no este restringido
        if(interesado.getRestringido()) {
            throw new InteresadoRestringidoException("El interesado esta restringido para probar vehiculos");
        }
    }
}
